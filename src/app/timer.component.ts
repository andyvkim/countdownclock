...
export class TimerComponent  {
    ...
    allowChange(){
     ...
     setTime(){
      ...
      if(this.arr[4]){
        this.hour=parseInt(this.arr[4]);
      }
      if(this.arr[5]){
        this.hour+=parseInt(this.arr[5])*10;
      }

     
      if(this.arr[2]){
        this.minute=parseInt(this.arr[2]);
      }
       if(this.arr[3]){
        this.minute+=parseInt(this.arr[3])*10;
      }

    
      if(this.arr[0]){
        this.second=parseInt(this.arr[0]);
      }
      if(this.arr[1]){
        this.second+=parseInt(this.arr[1])*10;
      }

      this.totalSeconds=this.hour*60*60+this.minute*60+this.second;
      this.remainingSecond=this.totalSeconds;
    }

    adjustTimer(){
      let s=this.second-this.adjust;
      if(s<0){

        if(this.minute>0){
           this.minute-=1;
           this.second=59;
          this.second-=this.adjust;
